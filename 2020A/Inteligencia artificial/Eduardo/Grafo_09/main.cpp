...
int main()
{
    ...
    cout << "Ejecutando recorrido de grafo..." << endl;
    indice=grafo->primero(v=grafo->dameVertice(1));
    cout << "indice es " << indice << endl;
    if (indice!=VERTICE_NULO){
        w=grafo->dameVertice(v,indice);
        cout << "primer vertice adyacente a " << v->numVertice << " es " << w->numVertice << endl;
        indice=grafo->siguiente(v,indice);
        w=grafo->dameVertice(v,indice);
        cout << "segundo vertice adyacente a " << v->numVertice << " es " << w->numVertice << endl;
    }
    else{
        cout << "no hay vertice adyacente a " << v->numVertice << endl;
    }
    pausar();
    delete grafo;
    pausar();
...
Vertice<EtiquetaVertice>* encuentraVerticeMinimo(Lista<Vertice<EtiquetaVertice>*>& verticesFaltantes,
    ...
    for(i=verticesFaltantes.siguiente(i);i!=j;i=verticesFaltantes.siguiente(i)){
        ...
        if ((distanciaMinima==ETIQUETA_ARCO_NULA) ||
            (distanciaPosible!=ETIQUETA_ARCO_NULA && distanciaPosible<distanciaMinima)){
...
EtiquetaArco encuentraDistanciaMinima(Vertice<EtiquetaVertice>* v,Vertice<EtiquetaVertice>* w,
    ...
    if ((dist1=distancias[w->numVertice-1])!=ETIQUETA_ARCO_NULA){
        dist2=grafo->etiquetaArco(w,v);
        if (dist2==ETIQUETA_ARCO_NULA){
            dist1=ETIQUETA_ARCO_NULA;  //distancia combinada es infinita
        }
        else{
            dist1+=dist2;
        }
    }
    if ((dist2=distancias[v->numVertice-1])!=ETIQUETA_ARCO_NULA){
        if (dist1==ETIQUETA_ARCO_NULA || dist2<dist1){
            dist1=dist2;
        }
        else{
...
void imprimeDistancias(Lista<TipoElemento>& verticesVerificados,Vertice<EtiquetaVertice>* verticeInspeccionado,
    ...
    for(i=1;i<NUM_VERTICES;i++){
        if (distancias[i]==ETIQUETA_ARCO_NULA){
            cout << "\tinf";
        }
        else{
            cout << "\t" << distancias[i];
