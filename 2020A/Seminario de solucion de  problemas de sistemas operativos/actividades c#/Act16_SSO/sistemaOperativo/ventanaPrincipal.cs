...
namespace sistemaOperativo
{
    public partial class ventanaPrincipal : Form
    {
        ...
        ArchivoSuspendidos archivoSuspendidos = new ArchivoSuspendidos();
        List<Pagina> paginasEnMemoriaVirtual = new List<Pagina>();
        int numeroDeQuantum = 0;
        int cantidadDeProcesosAEjecutar = 0;
        int contadorGlobal = 0;
        string teclaPresionada;
        int marcosDisponibles = 31;
                ...
                if (proceso.obtenerEstado() == "LISTO" || proceso.obtenerEstado() == "BLOQ")
                {
                    if (arregloSegmentos[segmento].obtenIdProceso() == proceso.obtenerId())
                    {
                        arregloSegmentos[segmento].fijaColor(proceso.obtenerEstado(),
                          ...
                          proceso.obtenerEstado() == "SUSPENDIDO")
                {
                    if (arregloSegmentos[segmento].obtenIdProceso() == proceso.obtenerId())
                    {
                        arregloSegmentos[segmento].fijaColor(proceso.obtenerEstado(),
        ...
        bool extraeProcesosDeMemoriaVirtual(Pagina procesoEnMV)
        {
            bool respuesta = false;
            int posicion = 31;
            int numeroMarco = posicion * 4;
            int desp = 0;
            int totalSegmentos = procesoEnMV.NumeroDeSegmentos;            

            while (posicion >= 0)
            {
                if (arregloSegmentos[numeroMarco].obtenIdProceso() == -1)
                {
                    respuesta = true;
                    desp = 0;
                    while (desp < totalSegmentos)
                    {
                        arregloSegmentos[numeroMarco + desp].fijaColor("LISTO", true);
                        arregloSegmentos[numeroMarco + desp].fijaIdProceso(procesoEnMV.IdProceso);
                        desp++;
                    }
                    marcosDisponibles--;
                    break;
                ...
                posicion--;
                numeroMarco = posicion * 4;
            }
            return respuesta;
        }
        
        void enviaProcesosAMemoriaVirtual()
        {
            int posicion = 31;
            int numeroMarco = posicion * 4;
            int nSegmentos = 0;
            ...
            {
                if (arregloSegmentos[numeroMarco].obtenIdProceso() != -1 &&
                    arregloSegmentos[numeroMarco].obtenColor() != "ROJO")
                {
                    var resultado =
                        paginasEnMemoriaVirtual
                        .Find(item => item.IdProceso == arregloSegmentos[numeroMarco].obtenIdProceso());
                    if (resultado == null)
                    {
                        Pagina nuevaPagina = new Pagina();
                        nuevaPagina.IdProceso = arregloSegmentos[numeroMarco].obtenIdProceso();
                        nSegmentos = 0;
                        desp = 0;
                        while (desp < 4)
                        {
                            if (arregloSegmentos[numeroMarco + desp].obtenIdProceso() != -1)
                            {
                                arregloSegmentos[numeroMarco + desp].fijaColor("", false);
                                arregloSegmentos[numeroMarco + desp].fijaIdProceso(-1);
                                nSegmentos++;
                            }
                            else
                                break;
                        ...
                        nuevaPagina.NumeroDeSegmentos = nSegmentos;
                        paginasEnMemoriaVirtual.Add(nuevaPagina);
                        marcosDisponibles++;
                    }
                }
                posicion--;
            ...
            Console.WriteLine("PROCESOS EN MEMORIA VIRTUAL");
            foreach (Pagina p in paginasEnMemoriaVirtual)
                Console.WriteLine("ID: {0}, No. Segmentos: {1} ", p.IdProceso, p.NumeroDeSegmentos);
        }

        void sinMarcosLibre(Pagina procesoEnMV, Proceso procesoEnMP)
        {
            int posicion = 31;
            int numeroMarco = posicion * 4;
            int desp = 0;
            int totalSegmentosEnMv = procesoEnMV.NumeroDeSegmentos;
            Console.WriteLine("ENTRE AL FOR");

            while (posicion >= 0)
            {
                if (arregloSegmentos[numeroMarco].obtenIdProceso() == procesoEnMP.obtenerId())
                {
                    Pagina nuevaPagina = new Pagina();
                    nuevaPagina.IdProceso = procesoEnMP.obtenerId();
                    desp = 0;
                    while (desp < 4)
                    {
                        if (desp < totalSegmentosEnMv)
                        {
                            arregloSegmentos[numeroMarco + desp].fijaColor("LISTO", true);
                            arregloSegmentos[numeroMarco + desp].fijaIdProceso(procesoEnMV.IdProceso);
                        }
                    ...
                    int totalSegmentosEnMP = Math.Abs(((procesoEnMP.obtenerTamanio() / 4) * 4) - procesoEnMP.obtenerTamanio());
                    if (totalSegmentosEnMP == 0)
                        totalSegmentosEnMP = 4;
                    nuevaPagina.NumeroDeSegmentos = totalSegmentosEnMP;
                    paginasEnMemoriaVirtual.Add(nuevaPagina);
                    break;
                }
                posicion--;
        ...
        async Task ejecutarEnMemoria(List<Proceso> procesosGenerados)
        {
            ...
            labelMemoriaVirtualTamanioMostrar.Text = paginasEnMemoriaVirtual.Count.ToString();
            ...
            while (contadorProcesosTerminados < procesosGenerados.Count || archivoSuspendidos.NumProcesosSuspendidos() > 0)
            {
                ...
                if (banderaPausaYContinua)
                {
                    ...
                    if (banSuspendeProceso)
                    {
                        int posicionProcesoSuspende = archivoSuspendidos.posicionProcesoSuspendido(procesoEjecutando);

                        if (posicionProcesoSuspende != -1)
                        {
                            var resultado =
                                paginasEnMemoriaVirtual
                                .Find(item => item.IdProceso == procesoEjecutando[posicionProcesoSuspende].obtenerId());
                            paginasEnMemoriaVirtual.Remove(resultado);
                            procesoEjecutando[posicionProcesoSuspende].fijarBanEjecutando(false);
                            procesoEjecutando[posicionProcesoSuspende].fijarEstado("SUSPENDIDO");
                            procesoEjecutando[posicionProcesoSuspende].fijarTiempoTranscurridoBloqueado(0);

                            actualizaTabla(procesoEjecutando[posicionProcesoSuspende]);
                            dibujaTabla();

                            archivoSuspendidos.guardaEnDisco(procesoEjecutando[posicionProcesoSuspende]);

                            procesoEjecutando.RemoveAt(posicionProcesoSuspende);

                            labelIDProcesoSuspendidoMostrar.Text = archivoSuspendidos.obtenPrimerProceso().obtenerId().ToString();
                            labelTamanioSuspendidoMostrar.Text = archivoSuspendidos.obtenPrimerProceso().obtenerTamanio().ToString();
                            ...
                            if (indiceListaProcesosListos < procesosGenerados.Count)
                            {
                                ...
                                if (canProcesosCalculados > 0)
                                {
                                    ...
                                    {
                                        if (procesoEjecutando[j].obtenerEstado() != "SUSPENDIDO")
                                            posicionInsertar++;
                                        else
                                            break;
                            ...
                            labelNumDeSuspendidosMostrar.Text = archivoSuspendidos.NumProcesosSuspendidos().ToString();
                        ...
                        banSuspendeProceso = false;
                    }
                    else if (banRetornaProcesoSuspendido)
                    {
                        if (archivoSuspendidos.NumProcesosSuspendidos() > 0)
                        {
                            int canProcesosCalculados = calculaNuevosProcesos(archivoSuspendidos.obtenProcesoEnArchivo(), 0);
                            if (canProcesosCalculados > 0)
                            {
                                int j = 0;
                                int posicionInsertar = 0;
                                ...
                                Proceso nuevoProceso = new Proceso();
                                nuevoProceso = archivoSuspendidos.obtenProcesoEnArchivo()[0];
                                nuevoProceso.fijarEstado(ESTADOS_PROCESO[1]);
                                procesoEjecutando.Insert(posicionInsertar,
                                                          archivoSuspendidos.obtenProcesoEnArchivo()[0]);
                                ...
                                archivoSuspendidos.actualizaLista();
                                archivoSuspendidos.actualizaArchivo();
                                ...
                                {
                                    labelIDProcesoSuspendidoMostrar.Text = "";
                                    labelTamanioSuspendidoMostrar.Text = "";
                                }
                            }
                        }

                        banRetornaProcesoSuspendido = false;
                    }
                    ...
                    if (procesoEjecutando.Count != 0)
                    {
                        if (procesoEjecutando[i].obtenerEstado() == ESTADOS_PROCESO[1])
                        {
                            ...
                            if (banderaError)
                            {
                                var resultado =
                                    paginasEnMemoriaVirtual
                                    .Find(item => item.IdProceso == procesoEjecutando[i].obtenerId());
                                paginasEnMemoriaVirtual.Remove(resultado);
                                ...
                                if (procesoEjecutando.Count != 0)
                                {
                                    ...
                                    if (res != null)
                                    {
                                        paginasEnMemoriaVirtual.Remove(res);
                                        if (extraeProcesosDeMemoriaVirtual(res)==false)
                                        {
                                            bool bandera = false;
                                            for (int indicePE = 1; indicePE < procesoEjecutando.Count; indicePE++)
                                            {
                                                var procesoEnMV =
                                                    paginasEnMemoriaVirtual
                                                    .Find(item => item.IdProceso == procesoEjecutando[indicePE].obtenerId());
                                                if ((procesoEjecutando[indicePE].obtenerEstado() == "LISTO" ||
                                                    procesoEjecutando[indicePE].obtenerEstado() == "BLOQ") &&
                                                    !procesoEjecutando[indicePE].obtenerBanEjecutando() &&
                                                    procesoEnMV == null)
                                                {
                                                    bandera = true;
                                                    paginasEnMemoriaVirtual.Remove(res);
                                                    sinMarcosLibre(res,procesoEjecutando[indicePE]);
                                                    break;
                                                }
                                            }
                                            if(bandera==false)
                                            Console.WriteLine("NO ENCONTRO ESPACIO**");
                                        }
                                    }
                            ...
                            if (banMemoriaVirtual)
                            {
                                if (paginasEnMemoriaVirtual.Count < 32)
                                {
                                    enviaProcesosAMemoriaVirtual();
                                    VentanaMemoriaVirtual ventanaMemoriaVirtual = new VentanaMemoriaVirtual();
                                    ventanaMemoriaVirtual.fijaLista(ref paginasEnMemoriaVirtual);
                                    ...
                                    labelMemoriaVirtualTamanioMostrar.Text = paginasEnMemoriaVirtual.Count.ToString();
                                    ventanaMemoriaVirtual.ShowDialog();
                                    ventanaMemoriaVirtual.Dispose();
                                }
                                else
                                {
                                    VentanaMemoriaVirtual ventanaMemoriaVirtual = new VentanaMemoriaVirtual();
                ...
                if (banderaBloquesDeControlDeProcesos == true && banderaPausaYContinua == false)
                {
                    foreach (Proceso bcpDeProceso in procesosGenerados)
                    {
                        ...
                        else if (bcpDeProceso.obtenerEstado() == "SUSPENDIDO" && bcpDeProceso.obtenerBanEjecutando() == false)
                        {
                            bcpDeProceso.fijarTiempoDeFinalizacion(contadorGlobal);
                            bcpDeProceso.fijarTiempoDeRetorno(bcpDeProceso.obtenerTiempoDeFinalizacion(), bcpDeProceso.obtenerTiempoDeLlegada());
                            bcpDeProceso.fijarTiempoDeEspera((bcpDeProceso.obtenerTiempoDeRetorno() - 1) - bcpDeProceso.obtenerTiempoDeServicio());
