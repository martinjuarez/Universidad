...
namespace sistemaOperativo
{
    public partial class ventanaPrincipal : Form
    {
        ...
        VirtualMemory memoriaVirtualVentana = new VirtualMemory();
        ...
        Dictionary<int, int> procesosAMemoriaVirtual()
        {
            ...
            while (posicion >= 0)
            {
                ...
                {
                    if( !dict.ContainsKey(arregloSegmentos[numeroMarco].obtenIdProceso()) )
                    {
                        nSegmentos = 0;
                        desp = 0;
                        while (desp < 4)
                        ...
                        dict.Add(arregloSegmentos[numeroMarco].obtenIdProceso(), nSegmentos);
                    }
                }
        ...
        async Task ejecutarEnMemoria(List<Proceso> procesosGenerados)
        {
            ...
            while (contadorProcesosTerminados < procesosGenerados.Count || archivoSuspendidos.NumProcesosSuspendidos() > 0)
            {
                ...
                if (banderaPausaYContinua)
                {
                    ...
                    {
                        Dictionary<int, int> memoriaVirtual = new Dictionary<int, int>();
                        memoriaVirtual = procesosAMemoriaVirtual();

                        foreach( KeyValuePair<int,int> item in memoriaVirtual )
                        {
                            Console.WriteLine("ID: {0}, Segmentos:{1}", item.Key, item.Value);
                        }

                        memoriaVirtualVentana.ShowDialog();
